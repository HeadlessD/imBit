...
//        [imagePickerVc showProgressHUD]; //dkq进入
...
- (void)initSubviews {
    ...
    if (!imagePickerVc.isPreview) {
        if (imagePickerVc.defaultAlbumName && !_model) {
            [imagePickerVc showAlertWithTitle:[NSString stringWithFormat:@"指定的相册\"%@\"不存在", imagePickerVc.defaultAlbumName] complete:^{
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
...
//        buttonX = CGRectGetMaxX(_previewButton.frame);
    ...
    if (imagePickerVc.allowPickingOriginalPhoto && imagePickerVc.isPreview==NO) {
        ...
        [_originalPhotoButton setImage:DFImage(imagePickerVc.photoOriginDefImageName) forState:UIControlStateDisabled];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((imagePickerVc.sortAscendingByCreateDate && indexPath.row >= _models.count) || (!imagePickerVc.sortAscendingByCreateDate && indexPath.row == 0)) && _showTakePhotoBtn) {
        ...
        cell.posterImage = DFImage(imagePickerVc.takePictureImageName);
    ...
    if (picker.sourceType==UIImagePickerControllerSourceTypeCamera && [mediaType isEqualToString:@"public.image"]){
        ...
        [[LFAssetManager manager] saveImageToCustomPhotosAlbumWithTitle:nil images:@[chosenImage] complete:^(NSArray<id> *assets, NSError *error) {
            ...
            if (assets && !error) {
                ...
                [imagePickerVc hideProgressHUD];
                [imagePickerVc showAlertWithTitle:nil message:error.localizedDescription complete:^{
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        [imagePickerVc showAlertWithTitle:nil cancelTitle:nil message:message complete:^{
            if (@available(iOS 8.0, *)){
                ...
                NSString *message = @"无法跳转到隐私设置页面，请手动前往设置页面";
