...
- (instancetype)initWithAccessType:(MSTImagePickerAccessType)accessType{
    if (self = [super init]) {
        ...
        self.albumTitle = NSLocalizedStringFromTable(@"str_photos", @"MSTImagePicker", LLSTR(@"104000"));
...
- (BOOL)addSelectedAsset:(MSTAssetModel *)asset {
    if (self.pickedModelIdentifiers.count == self.config.maxSelectCount) {
        ...
        [self presentViewController:[self addAlertControllerWithTitle:[NSString stringWithFormat:NSLocalizedStringFromTable(@"str_get_to_maximum_selected", @"MSTImagePicker", @"最大选择数量提示"), self.config.maxSelectCount] actionTitle:NSLocalizedStringFromTable(@"str_i_see", @"MSTImagePicker", LLSTR(@"101023"))] animated:YES completion:nil];
...
- (void)didFinishPicking:(BOOL)isFullImage {
    ...
    for (MSTAssetModel *model in self.pickedModels) {
        ...
        [[MSTPhotoManager defaultManager] getPickingImageFromPHAsset:model.asset isFullImage:isFullImage maxImageWidth:self.config.maxImageWidth completionBlock:^(UIImage *image, UIImage *orignalImage, NSDictionary *info) {
            pickingModel.image = image;
            pickingModel.orignalImage = orignalImage;
...
- (void)mp_setupToolBarButtonEnbled {
    ...
    [self.originalImageButton setEnabled:YES];
    [self.originalTextButton setEnabled:YES];
    ...
    //    [self.originalTextButton setSelected:YES];
    //}
}
...
- (MSTPhotoGridController *)photoGridController{
    if (!_photoGridController) {
        ...
        if (_isHideFullButtonAndImg) {
            self.photoGridController.isHideFullButtonAndImgPhotoGrid = YES;
        }else{
            self.photoGridController.isHideFullButtonAndImgPhotoGrid = NO;
        }
    }
...
- (UIButton *)previewButton {
    if (!_previewButton) {
        ...
        [_previewButton setTitleColor:THEME_GRAY forState:UIControlStateDisabled];
...
- (UIButton *)originalImageButton {
    if (!_originalImageButton) {
        ...
        _originalImageButton.frame = CGRectMake(5, 5, 34, 34);
...
- (UIButton *)originalTextButton {
    if (!_originalTextButton) {
        ...
        [_originalTextButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
...
- (UIButton *)doneButton {
    if (!_doneButton) {
        ...
        _doneButton.frame = CGRectMake(self.toolbar.mst_width-[self mp_calculateWidthWithString:sendStr textSize:17]-20, 0, [self mp_calculateWidthWithString:sendStr textSize:17] + 20, 44);
        [_doneButton setTitle:sendStr forState:UIControlStateNormal];
        [_doneButton setTitleColor:THEME_DARKBLUE forState:UIControlStateDisabled];
...
- (void)setMaxImageWidth:(CGFloat)maxImageWidth {
    if (maxImageWidth < 600) maxImageWidth = 600;
...
- (void)setIsHideFullButtonAndImg:(BOOL)isHideFullButtonAndImg{
    if (isHideFullButtonAndImg) {
        self.originalTextButton.hidden = YES;
        self.originalImageButton.hidden = YES;
        _isHideFullButtonAndImg = isHideFullButtonAndImg;
        
        self.albumListController.isHideFullButtonAndImgAlbumListController = YES;
    }
}
