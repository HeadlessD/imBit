...
//加载“最近添加”
- (void)loadCurrentAddInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumRecentlyAdded options:nil];
...
//加载“连拍快照”
- (void)loadBurstsInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumBursts options:nil];
...
//加载“延时视频”
- (void)loadTimelapsesInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumTimelapses options:nil];
...
//加载“全景照片”
- (void)loadPanoramasInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumPanoramas options:nil];
...
//加载"屏摄照片”
- (void)loadScreenshotsInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumScreenshots options:nil];
...
//加载"深景照片”
- (void)loadDepthEffectInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    if (@available(iOS 10.2, *)) {
        PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumDepthEffect options:nil];
...
//加载"live照片”
- (void)loadLivePhotosInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    if (@available(iOS 10.3, *)) {
        PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumLivePhotos options:nil];
...
//加载"深景照片”
- (void)loadAnimatedInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    if (@available(iOS 11.0, *)) {
        PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumAnimated options:nil];
...
//加载"长曝光照片”
- (void)loadLongExposuresInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    if (@available(iOS 11.0, *)) {
        PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumLongExposures options:nil];
...
//加载"我的收藏照片”
- (void)loadFavoritesInfoisDesc:(BOOL)isDesc isShowEmpty:(BOOL)isShowEmpty isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(MSTAlbumModel *))completionBlock {
    if (@available(iOS 11.0, *)) {
        PHFetchResult *albumCollection= [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumFavorites options:nil];
...
- (void)loadAlbumInfoIsShowEmpty:(BOOL)isShowEmpty isDesc:(BOOL)isDesc isOnlyShowImage:(BOOL)isOnlyShowImage CompletionBlock:(void (^)(PHFetchResult *, NSArray *))completionBlock {
    ...
    [self loadCameraRollInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“最近添加”
    [self loadCurrentAddInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];

    //读取“连拍快照”
    [self loadBurstsInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“延时视频”
    [self loadTimelapsesInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“全景视频”
    [self loadPanoramasInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];

    //读取“屏摄照片”
    [self loadScreenshotsInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“深景照片”
    [self loadDepthEffectInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“Live照片”
    [self loadLivePhotosInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“动画照片”
    [self loadAnimatedInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“长曝光照片”
    [self loadLongExposuresInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
    
    //读取“我的收藏”
    [self loadFavoritesInfoisDesc:isDesc isShowEmpty:isShowEmpty isOnlyShowImage:isOnlyShowImage CompletionBlock:^(MSTAlbumModel *result) {
        if (result != nil)[albumModelsArray addObject:result];
    }];
...
- (void)getPickingImageFromPHAsset:(PHAsset *)asset isFullImage:(BOOL)isFullImage maxImageWidth:(CGFloat)width completionBlock:(void (^)(UIImage *, UIImage *, NSDictionary *))completionBlock {
    PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
    ...
    if (isFullImage) {
        ...
        if (aspectRatio >= 1.0)
        {
            CGFloat pixelHeight = width;
            CGFloat pixelWidth = width * aspectRatio;
    ...
    [self.imageManager requestImageForAsset:asset targetSize:targetSize contentMode:PHImageContentModeAspectFit options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
        ...
        UIImage *image = [result scaleImageWithMaxWidth:width];
        UIImage *origanlImage = nil;
        if (isFullImage)
            origanlImage = result;
        
        completionBlock ? completionBlock(image, origanlImage, info) : nil;
    }];
}
