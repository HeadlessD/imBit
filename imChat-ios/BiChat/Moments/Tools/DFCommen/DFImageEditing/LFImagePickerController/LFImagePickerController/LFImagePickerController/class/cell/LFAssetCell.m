...
- (void)initUI {
    ...
    [editMaskImageView setImage:DFImage(@"contacts_add_myablum")];
...
- (void)selectPhoto:(BOOL)isSelected index:(NSUInteger)index animated:(BOOL)animated
{
    ...
    if (_selectPhotoButton.selected) {
        ...
        image = [UIImage lf_mergeImage:DFImage(self.photoSelImageName) text:text];
    } else {
        image = DFImage(self.photoDefImageName);
    }
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:DFImage(@"fileicon_video_wall")];
