...
@interface GroupSetupViewController ()
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Configure the cell...
    if ([[BiChatGlobal sharedManager].uid isEqualToString:[self.groupProperty objectForKey:@"ownerUid"]] && [[self.groupProperty objectForKey:@"virtualGroupId"]length] == 0)
    {
    ...
    else
    {
        ...
        {
            //是管理员
            if ([BiChatGlobal isMeGroupOperator:self.groupProperty] && [[self.groupProperty objectForKey:@"virtualGroupId"]length] == 0)
            {
                cell.textLabel.text = LLSTR(@"201219");
                cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    ...
    if (indexPath.section == 1 && indexPath.row == 0)
    {
        ...
        if ([BiChatGlobal isMeGroupOperator:self.groupProperty] && [[self.groupProperty objectForKey:@"virtualGroupId"]length] == 0)
        {
            cell.textLabel.text = LLSTR(@"201308");
            
            if ([[self.groupProperty objectForKey:@"isUnlimitedGroup"]boolValue])
                cell.detailTextLabel.text = LLSTR(@"201333");
            else
            {
                UISwitch *switch4CanChangeGroupName = [[UISwitch alloc]initWithFrame:CGRectMake(self.view.frame.size.width - 66, 6, 100, 30)];
                [switch4CanChangeGroupName addTarget:self action:@selector(onSwitchCanChangeGroupName:) forControlEvents:UIControlEventValueChanged];
                switch4CanChangeGroupName.on = [[self.groupProperty objectForKey:@"changeNameRightOnly"]boolValue];
                [cell.contentView addSubview:switch4CanChangeGroupName];
            }
        }
        else
        {
    ...
    else if (indexPath.section == 2 && indexPath.row == 0)
    {
        ...
        {
            NSString * userCount = [NSString stringWithFormat:@"%@",[self.groupProperty objectForKey:@"groupUserCount"]];
            cell.detailTextLabel.text = [LLSTR(@"201327") llReplaceWithArray:@[userCount]];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    else if (indexPath.section == 2 && indexPath.row == 1)
    {
        ...
        if ([BiChatGlobal isMeGroupOperator:self.groupProperty] &&
            ...
            [[self.groupProperty objectForKey:@"virtualGroupId"]length] == 0)
        {
            cell.textLabel.text = LLSTR(@"201314");
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        }
    }
    else if (indexPath.section == 2 && indexPath.row == 2)
    {
        cell.textLabel.text = LLSTR(@"201316");
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
    else if (indexPath.section == 3 && indexPath.row == 0)
    {
        cell.textLabel.text = LLSTR(@"201318");
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%ld", (long)[[self.groupProperty objectForKey:@"groupBlockUserLevelTwo"]count]];
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
    else if (indexPath.section == 4 && indexPath.row == 0)
    {
        cell.textLabel.text = LLSTR(@"201320");
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%ld", (long)[[self.groupProperty objectForKey:@"muteUsers"]count]];
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
    ...
    if ([BiChatGlobal isMeGroupOperator:_groupProperty])
    {
        ...
        else if (indexPath.section == 0 && indexPath.row == 1)
        {
            if ([[BiChatGlobal sharedManager].uid isEqualToString:[self.groupProperty objectForKey:@"ownerUid"]])
            {
                ...
                wnd.showMemo = YES;
                wnd.canSelectMax = 10;
                wnd.beyondSelectMaxAlert = LLSTR(@"301028");
                UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:wnd];
                nav.navigationBar.translucent = NO;
                nav.navigationBar.tintColor = THEME_COLOR;
                [self.navigationController presentViewController:nav animated:YES completion:nil];
        ...
        else if (indexPath.section == 2 && indexPath.row == 1)
        {
            if ([BiChatGlobal isMeGroupOperator:self.groupProperty] &&
                ![[self.groupProperty objectForKey:@"isUnlimitedGroup"]boolValue])
            {
                [self upgrade2BigGroup];
            }
        }
        else if (indexPath.section == 2 && indexPath.row == 2)
        {
            [self upgrade2VirtualGroup];
        }
        else if (indexPath.section == 3 && indexPath.row == 0)
        {
...
- (void)memberSelected:(NSArray *)member withCookie:(NSInteger)cookie
{
            ...
            [NetworkModule delGroupAssistant:self.groupId assistant:array4Delete completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                ...
                if (success)
                {
                    ...
                    if (array4DeletedAssistant.count > 0)
                    {
                        ...
                        NSString *msgId = [BiChatGlobal getUuidString];
                        NSMutableDictionary *sendData = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", MESSAGE_CONTENT_TYPE_DELASSISTANT], @"type",
                                                         [array4DeletedAssistant JSONString], @"content",
                                                         self.groupId, @"receiver",
                                                         [self.groupProperty objectForKey:@"groupName"], @"receiverNickName",
                                                         [BiChatGlobal getGroupAvatar:self.groupProperty], @"receiverAvatar",
                                                         [BiChatGlobal sharedManager].uid, @"sender",
                ...
                if (success)
                {
                    ...
                    if (array4NewVIP.count > 0)
                    {
                        ...
                        NSMutableDictionary *sendData = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", MESSAGE_CONTENT_TYPE_ADDVIP], @"type",
                                                         [array4NewVIP JSONString], @"content",
                                                         self.groupId, @"receiver",
                                                         [self.groupProperty objectForKey:@"groupName"], @"receiverNickName",
                                                         [BiChatGlobal getGroupAvatar:self.groupProperty], @"receiverAvatar",
                                                         [BiChatGlobal sharedManager].uid, @"sender",
                            ...
                            if (success)
                            {
                                ...
                                if (array4DeletedVIP.count > 0)
                                {
                                    ...
                                    NSMutableDictionary *sendData = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", MESSAGE_CONTENT_TYPE_DELVIP], @"type",
                                                                     [array4DeletedVIP JSONString], @"content",
                                                                     self.groupId, @"receiver",
                                                                     [self.groupProperty objectForKey:@"groupName"], @"receiverNickName",
                                                                     [BiChatGlobal getGroupAvatar:self.groupProperty], @"receiverAvatar",
                                                                     [BiChatGlobal sharedManager].uid, @"sender",
    ...
    else if (cookie == 4)       //群成员迁移第一步
    {
        ...
        wnd.canShowBlock = ^BOOL(NSString *groupId) {
            ...
            //是否同一个群
            if ([[_groupProperty objectForKey:@"virtualGroupId"]length] == 0 &&
                [self.groupId isEqualToString:groupId])
                return NO;
            ...
            {
                NSLog(@"not same group operation");
                return NO;
            }
...
//切换是否收费群
- (void)onSwitchChargeGroup:(id)sender
{
    UISwitch *s = (UISwitch *)sender;
    
    //设置高级消息
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:s.on], @"payGroup", nil];
    [BiChatGlobal ShowActivityIndicator];
    [NetworkModule setGroupPublicProfile:self.groupId profile:dict completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        ...
        if (success)
        {
            [self.groupProperty setObject:[NSNumber numberWithBool:s.on] forKey:@"payGroup"];
            [self.tableView reloadData];
        }
        else
        {
            [BiChatGlobal showInfo:LLSTR(@"301003") withIcon:nil];
            [[BiChatGlobal sharedManager]imChatLog:@"----network error - 18", nil];
        }
    }];
}
...
- (void)onSwitchCanChangeGroupName:(id)sender
{
    ...
    [NetworkModule setGroupPublicProfile:self.groupId profile:dict completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        ...
        {
            [BiChatGlobal showInfo:LLSTR(@"301003") withIcon:nil];
            [[BiChatGlobal sharedManager]imChatLog:@"----network error - 19", nil];
        }
    }];
}
...
- (void)setCanAddMember:(BOOL)canAddMember
{
    ...
    [NetworkModule setGroupPublicProfile:self.groupId profile:dict completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        ...
        {
            [BiChatGlobal showInfo:LLSTR(@"301003") withIcon:nil];
            [[BiChatGlobal sharedManager]imChatLog:@"----network error - 20", nil];
        }
    }];
}
        ...
        group_friends_selected:(NSMutableArray *)group_friends_selected
{
    ...
    [NetworkModule rejectGroupApplication:groupId userList:group_friends_selected completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        ...
        if (success)
        {
            ...
            NSArray *array4AllApply = [BiChatGlobal sharedManager].array4ApproveList;
            for (NSDictionary *item in array4AllApply)
            {
...
- (void)upgrade2BigGroup
{
    ...
    //开始升级大大群
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LLSTR(@"201314")
                                                                             message:[NSString stringWithFormat:@"\r\n%@", LLSTR(@"201315")]
                                                                      preferredStyle:UIAlertControllerStyleActionSheet];
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:LLSTR(@"101001") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if ([[self.groupProperty objectForKey:@"addNewMemberRightOnly"]boolValue])
        {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LLSTR(@"203003")
                                                                                     message:[NSString stringWithFormat:@"\r\n%@", LLSTR(@"201343")]
                                                                              preferredStyle:UIAlertControllerStyleActionSheet];
            UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:LLSTR(@"101001") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                
                //开始关闭群聊邀请确认
                [self setCanAddMember:NO];
                [self upgrade2BigGroupInternal];
            }];
...
- (void)upgrade2BigGroupInternal
{
    //开始设置为虚拟群
    [BiChatGlobal ShowActivityIndicator];
    self.tableView.userInteractionEnabled = NO;
    [NetworkModule upgradeToBigGroup:self.groupId completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        
        [BiChatGlobal HideActivityIndicator];
        self.tableView.userInteractionEnabled = YES;
        if (success)
        {
            [BiChatGlobal showInfo:LLSTR(@"301735") withIcon:[UIImage imageNamed:@"icon_OK"]];
            [self.groupProperty setObject:[NSNumber numberWithInteger:1000000] forKey:@"groupUserCount"];
            [self.groupProperty setObject:[NSNumber numberWithBool:YES] forKey:@"isUnlimitedGroup"];
            [self.groupProperty setObject:[NSNumber numberWithBool:YES] forKey:@"changeNameRightOnly"];
            [self.groupProperty setObject:[NSNumber numberWithBool:YES] forKey:@"dingRightOnly"];
            [self.groupProperty setObject:[NSNumber numberWithBool:YES] forKey:@"onlyAssistantCanAddFriend"];
            [self.tableView reloadData];
                        
            //同时要发送一条数据通知群中的其他成员
            NSString *msgId = [BiChatGlobal getUuidString];
            NSMutableDictionary *sendData = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", MESSAGE_CONTENT_TYPE_UPGRADETOBIGGROUP], @"type",
                                             @"", @"content",
                                             self.groupId, @"receiver",
                                             [self.groupProperty objectForKey:@"groupName"], @"receiverNickName",
                                             [BiChatGlobal getGroupAvatar:self.groupProperty], @"receiverAvatar",
...
- (void)upgrade2VirtualGroup
{
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:LLSTR(@"101001") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [BiChatGlobal ShowActivityIndicator];
        self.tableView.userInteractionEnabled = NO;
        [NetworkModule createVirtualGroup:self.groupId completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
            ...
            if (success)
            {
                ...
                [NetworkModule getGroupProperty:self.groupId completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                    ...
                    {
                        //修改本地的
                        if ([data objectForKey:@"virtualGroupId"] != nil)
                            [self.groupProperty setObject:[data objectForKey:@"virtualGroupId"] forKey:@"virtualGroupId"];
                        if ([data objectForKey:@"virtualGroupNum"] != nil)
                            [self.groupProperty setObject:[data objectForKey:@"virtualGroupNum"] forKey:@"virtualGroupNum"];
                        if ([data objectForKey:@"virtualGroupSubList"] != nil)
                            [self.groupProperty setObject:[data objectForKey:@"virtualGroupSubList"] forKey:@"virtualGroupSubList"];
                        [self.tableView reloadData];
                        ...
                        [NetworkModule sendMessageToGroup:self.groupId message:sendData completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                            if (success)
                            {
                                ...
                                //还要再发一条数据通知消息到0群
                                NSString *subGroup0Id = [[[self.groupProperty objectForKey:@"virtualGroupSubList"]firstObject]objectForKey:@"groupId"];
                                [NetworkModule getGroupProperty:subGroup0Id completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                                    if (success)
                                    {
                                        NSString *msgId = [BiChatGlobal getUuidString];
                                        NSMutableDictionary *sendData = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", MESSAGE_CONTENT_TYPE_CREATEVIRTUALGROUP], @"type",
                                                                         @"", @"content",
                                                                         subGroup0Id, @"receiver",
                                                                         [self.groupProperty objectForKey:@"groupName"], @"receiverNickName",
                                                                         [BiChatGlobal getGroupAvatar:self.groupProperty], @"receiverAvatar",
                                                                         [BiChatGlobal sharedManager].uid, @"sender",
                                                                         [BiChatGlobal sharedManager].nickName, @"senderNickName",
                                        ...
                                        [NetworkModule sendMessageToGroup:subGroup0Id message:sendData completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                                            if (success)
                                            {
                                                [[BiChatDataModule sharedDataModule]setLastMessage:subGroup0Id
                                                                                      peerUserName:@""
                                                                                      peerNickName:[self.groupProperty objectForKey:@"groupName"]
                                                                                        peerAvatar:[BiChatGlobal getGroupAvatar:self.groupProperty]
                                                                                           message:[BiChatGlobal getMessageReadableString:sendData groupProperty:self.groupProperty]
                                                ...
                                                //加入本地一条消息
                                                [[BiChatDataModule sharedDataModule]addChatContentWith:subGroup0Id content:sendData];
                                            }
                                        }];
                                    }
                                }];
