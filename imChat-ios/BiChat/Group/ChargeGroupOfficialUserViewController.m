//
//  ChargeGroupOfficialUserViewController.m
//  BiChat
//
...
@interface ChargeGroupOfficialUserViewController ()

@end

@implementation ChargeGroupOfficialUserViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = LLSTR(@"204004");
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //转正按钮
    UITableViewRowAction *officialAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:LLSTR(@"204007") handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
...
- (void)onButtonOKExtendMemberShip:(id)sender
{
    ...
    [NetworkModule extentChargeGroupTrailTimeStamp:self.groupId
                                   ...
                                   extendTimeStamp:[selectedTime timeIntervalSinceDate:expiredTime] * 1000
                                    completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                                        
                                        if (success)
                                        {
                                            [BiChatGlobal showInfo:LLSTR(@"204011") withIcon:[UIImage imageNamed:@"icon_OK"]];
                                            ...
                                            [MessageHelper sendGroupMessageToOperator:self.groupId type:MESSAGE_CONTENT_TYPE_CHARGEGROUPMEMBER content:[content mj_JSONString] needSave:YES needSend:YES completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                                            }];
                                        ...
                                        {
                                            [BiChatGlobal HideActivityIndicator];
                                            [BiChatGlobal showInfo:LLSTR(@"204012") withIcon:[UIImage imageNamed:@"icon_alert"]duration:ALERT_MESSAGE_DURATION enableClick:YES];
                                        }
                                    }];
}
...
- (void)processGroupUserList
{
    ...
    for (NSMutableDictionary *item in [self.groupProperty objectForKey:@"groupUserList"])
    {
        if (![[self.groupProperty objectForKey:@"groupTrailUids"]containsObject:[item objectForKey:@"uid"]] &&
            ![[self.groupProperty objectForKey:@"ownerUid"]isEqualToString:[item objectForKey:@"uid"]])
        {
