            ...
            resendAction:(SEL)resendAction
{
    ...
    UIView *view4Bk = [[UIView alloc]initWithFrame:CGRectMake((cellWidth - rect.size.width - 20) / 2, 0, (int)rect.size.width + 20, (int)rect.size.height + 7)];
    //view4Bk.backgroundColor = [UIColor colorWithWhite:.78 alpha:1];
    view4Bk.backgroundColor = [UIColor clearColor];
    view4Bk.layer.cornerRadius = 5;
    view4Bk.clipsToBounds = YES;
    [contentView addSubview:view4Bk];
    
    UILabel *label4Hint = [[UILabel alloc]initWithFrame:CGRectMake((cellWidth - rect.size.width) / 2, 0, rect.size.width, rect.size.height + 7)];
