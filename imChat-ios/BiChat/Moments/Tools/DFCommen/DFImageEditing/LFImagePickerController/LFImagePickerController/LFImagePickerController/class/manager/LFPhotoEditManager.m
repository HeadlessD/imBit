               ...
               completion:(void (^)(LFResultImage *resultImage))completion
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        bigImage = photoEdit.editPreviewImage;
        ...
        if (source.size.width > 700 && source.size.height > 700) {
            if (source.size.width > source.size.height) {
                imsize = CGSizeMake(source.size.width/source.size.height*700, 700);
            }else{
                imsize = CGSizeMake(700, source.size.height/source.size.width*700);
            }
        }else{
            imsize = CGSizeMake(source.size.width,source.size.height);
            bigImage = nil;
        }
        UIImage *biaozhunImage = source;
        ...
        if (isGif) { /** GIF图片处理方式 */
            ...
            biaozhunImage= source;
        } else {
            biaozhunImage = [source imageWithSize:imsize];
            NSData * testData = [source lf_fastestCompressImageDataWithSize:(compressSize <=0 ? kCompressSize : compressSize) imageSize:sourceData.length];
...
//                thumbnail = [source lf_scaleToSize:CGSizeMake(th_pixelWidth, th_pixelHeight)];
        ...
        result.originalImage = biaozhunImage;
