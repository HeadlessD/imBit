//
//  ChargeGroupTrailUserViewController.m
//  BiChat
//
...
@interface ChargeGroupTrailUserViewController ()

@end

@implementation ChargeGroupTrailUserViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = LLSTR(@"204003");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        label4ExpiredTime.text = LLSTR(@"204120");
    }
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //转正按钮
    UITableViewRowAction *officialAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:LLSTR(@"204006") handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        [button4Cancel addTarget:self action:@selector(onButtonCancelFreeMemberShip:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [button4OK addTarget:self action:@selector(onButtonOKFreeMemberShip:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)onButtonCancelFreeMemberShip:(id)sender
{
    [BiChatGlobal dismissModalViewFromBottom];
}

- (void)onButtonOKFreeMemberShip:(id)sender
{
    //开始延展用户的过期时间
    [BiChatGlobal dismissModalViewFromBottom];
    ...
    [BiChatGlobal ShowActivityIndicator];
    [NetworkModule extentChargeGroupTrailTimeStamp:self.groupId uids:@[[currentSelectedUserInfo objectForKey:@"uid"]] extendTimeStamp:[selectedTime timeIntervalSinceDate:expiredTime] * 1000 completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        
        if (success)
        {
            [BiChatGlobal showInfo:LLSTR(@"204009") withIcon:[UIImage imageNamed:@"icon_OK"]];
        ...
        {
            [BiChatGlobal HideActivityIndicator];
            [BiChatGlobal showInfo:LLSTR(@"204010") withIcon:[UIImage imageNamed:@"icon_alert"]duration:ALERT_MESSAGE_DURATION enableClick:YES];
        }
   }];
}
...
- (void)processGroupUserList
{
    ...
    {
        if ([[self.groupProperty objectForKey:@"groupTrailUids"]containsObject:[item objectForKey:@"uid"]])
        {
