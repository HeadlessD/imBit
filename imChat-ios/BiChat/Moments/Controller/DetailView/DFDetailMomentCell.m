...
@interface DFDetailMomentCell()<DFLikeCommentToolbarDelegate, DFLikeCommentViewDelegate,DFShareNewsViewDelegate,DFGridImageViewDelegate>
...
@implementation DFDetailMomentCell
...
//        [_playView setFrame:CGRectMake((sizeTwo.width - 70)/2, (sizeTwo.height - 70)/2, 70, 70)];
    ...
    //评论权限
    if (self.baseItem.message.isFriend) {
...
-(void)playVideoClick{
    if (_delegate != nil && [_delegate respondsToSelector:@selector(DFDetailMomentCellToPlayVideoWithMoment:)]) {
        [_delegate DFDetailMomentCellToPlayVideoWithMoment:self.baseItem];
    }
}
...
{
    CGFloat height = [DFDetailMomentCell getCellHeight:item];
    return height;
}
...
{
    if (_delegate != nil && [_delegate respondsToSelector:@selector(clickImgOnDFDetailMomentCellWithThumbImgArr:displayImgArr:withTag:withBaseModel:)]) {
        [_delegate clickImgOnDFDetailMomentCellWithThumbImgArr:thumbImgArr displayImgArr:displayImgArr withTag:tag withBaseModel:self.baseItem];
    }
}
...
{
    _likeCommentToolbar.hidden = !_likeCommentToolbar.hidden;
}
...
-(MLLinkClickLabel *)textContentYYTextView{
    if (!_textContentYYTextView) {
        ...
        __block DFDetailMomentCell * detailMomentCell = self;
        
        [_textContentYYTextView setDidClickLinkBlock:^(MLLink *link, NSString *linkText, MLLinkLabel *label) {
                //    NSLog(@"%@",link.linkValue);
            if ([[link.linkValue substringToIndex:3] isEqualToString:@"url"]){
                if (_delegate != nil && [_delegate respondsToSelector:@selector(pushWithUrlFromeDFDetailMomentCell:)]) {
                    [detailMomentCell.delegate pushWithUrlFromeDFDetailMomentCell:[link.linkValue substringFromIndex:3]];
                }
            }
        }];
