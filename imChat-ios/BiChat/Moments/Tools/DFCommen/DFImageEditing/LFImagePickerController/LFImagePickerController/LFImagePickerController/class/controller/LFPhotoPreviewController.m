...
CGFloat const toolbarDefaultHeight = 44.f + 5.0f;
...
- (void)configCustomNaviBar {
    ...
    if (self.isPhotoPreview) {
        ...
        UIImage *image = DFImage(@"navigationbar_back_arrow");
    ...
    [_selectButton setImage:DFImage(imagePickerVc.photoDefImageName) forState:UIControlStateNormal];
    [_selectButton setImage:DFImage(imagePickerVc.photoSelImageName) forState:UIControlStateSelected];
...
- (void)configBottomToolBar {
    ...
    if (imagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setImage:DFImage(imagePickerVc.photoOriginDefImageName) forState:UIControlStateNormal];
        [_originalPhotoButton setImage:DFImage(imagePickerVc.photoOriginSelImageName) forState:UIControlStateSelected];
...
//        NSString *text = [NSString stringWithFormat:@"1", [imagePickerVc.selectedModels indexOfObject:model]+1];
//        UIImage *image = [UIImage lf_mergeImage:DFImage(imagePickerVc.photoNumberIconImageName) text:text];
        [_selectButton setImage:[UIImage imageNamed:@"df_pic_select"] forState:UIControlStateSelected];
    }
    ...
    if (imagePickerVc.maxImagesCount != imagePickerVc.maxVideosCount) {
        if (imagePickerVc.selectedModels.count && model.type != imagePickerVc.selectedModels.firstObject.type) {
            if (model.type == LFAssetMediaTypePhoto) {
                _naviTipsLabel.text = @"选择视频时不能选择图片";
            } else {
                _naviTipsLabel.text = @"选择图片时不能选择视频";
            }
        } else {
            if (model.type == LFAssetMediaTypeVideo) {
