...
#import "MessageRecallCell.h"
#import "JSONKit.h"

@implementation MessageRecallCell
        ...
        if ([[message objectForKey:@"sender"]isEqualToString:[BiChatGlobal sharedManager].uid])
        {
            if ([[message objectForKey:@"content"]length] > 0)
            {
                ...
                if (dict == nil)
                {
                    ...
                    [str addAttribute:NSForegroundColorAttributeName value:THEME_DARKBLUE range:NSMakeRange(str.length - LLSTR(@"203102").length, LLSTR(@"203102").length)];
