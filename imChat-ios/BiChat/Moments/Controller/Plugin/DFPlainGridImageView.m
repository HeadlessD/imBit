...
-(void) initView
{
    ...
    for (int row=0; row<3; row++) {
        for (int column=0; column<3; column++) {
            ...
            DFImageUnitView *imageUnitView = [[DFImageUnitView alloc] initWithFrame:CGRectZero];
            ...
            imageUnitView.tag = row*3+column;
...
-(void) onClickImage:(UITapGestureRecognizer *)sender
{
    if (_delegate && [_delegate respondsToSelector:@selector(onClick:)]) {
        [_delegate onClick:sender.view.tag];
    }
}
...
-(void) onLongPress:(UILongPressGestureRecognizer *) recognizer
{
    if (recognizer.state == UIGestureRecognizerStateBegan){
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(onLongPress:)]) {
            [_delegate onLongPress:view.tag];
        }
    }
}
