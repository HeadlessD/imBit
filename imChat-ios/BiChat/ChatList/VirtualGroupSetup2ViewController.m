...
@interface VirtualGroupSetup2ViewController ()

@end

@implementation VirtualGroupSetup2ViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.navigationItem.title = [LLSTR(@"201201") llReplaceWithArray:@[[NSString stringWithFormat:@"%lu", [[self.groupProperty objectForKey:@"joinedGroupUserCount"]integerValue]]]];
            ...
            [[[[self.groupProperty objectForKey:@"virtualGroupSubList"]firstObject]objectForKey:@"groupId"]isEqualToString:self.groupId])
        {
            ...
            if (userCount < array4GroupMember.count || [[self.groupProperty objectForKey:@"isUnlimitedGroup"]boolValue])
            {
                ...
                if ([[self.groupProperty objectForKey:@"isUnlimitedGroup"]boolValue])
                    [button4More setTitle:[LLSTR(@"201410") llReplaceWithArray:@[[NSString stringWithFormat:@"%ld", [[self.groupProperty objectForKey:@"joinedGroupUserCount"]integerValue]]]]
...
- (void)contactSelected:(NSInteger)cookie contacts:(NSArray *)contacts
{
    ...
    //如果是超大群，直接发消息(虚拟群不可能是超大群，所以本分支可以废弃)
    if ([[self.groupProperty objectForKey:@"isUnlimitedGroup"]boolValue]);
    ...
    [NetworkModule addGroupMember:contacts groupId:self.groupId source:@{@"source":@"INVITE",@"inviter":[BiChatGlobal sharedManager].uid} completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
        ...
        if (success)
        {
            ...
            if (array4PeersSuccess.count > 0)
            {
                ...
                //向所有的虚拟子群里面发消息通知增加了管理员
                for (int i = 1; i < [[self.groupProperty objectForKey:@"virtualGroupSubList"]count]; i ++)
                {
                    [MessageHelper sendGroupMessageTo:[[[self.groupProperty objectForKey:@"virtualGroupSubList"]objectAtIndex:i]objectForKey:@"groupId"] type:MESSAGE_CONTENT_TYPE_ADDASSISTANT content:[array4PeersSuccess mj_JSONString] needSave:YES needSend:YES completedBlock:^(BOOL success, BOOL isTimeOut, NSInteger errorCode, id  _Nullable data) {
                    }];
                }
            }
            ...
            if (array4PeersAlreadyInWaitingPayList.count > 0)
            {
                ...
                for (NSString *uid in array4PeersAlreadyInWaitingPayList)
                {
...
- (void)onButtonQuit:(id)sender
{
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:LLSTR(@"101003") style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        //发送退出群聊消息message
        NSString *msgId = [BiChatGlobal getUuidString];
        NSDictionary *sendData = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d", MESSAGE_CONTENT_TYPE_QUITGROUP], @"type",
                                  @"", @"content",
                                  self.groupId, @"receiver",
                                  [BiChatGlobal getGroupNickName:self.groupProperty defaultNickName:nil], @"receiverNickName",
                                  [BiChatGlobal getGroupAvatar:self.groupProperty], @"receiverAvatar",
...
- (void)tapUserAvatar:(UITapGestureRecognizer *)tagGest
{
    ...
    wnd.inviterId = [info4Target objectForKey:@"inviter"];
    wnd.groupProperty = self.groupProperty;
