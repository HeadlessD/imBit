...
- (void)photoStartLoad
{
    ...
    {
        [_imageView yy_setImageWithURL:[NSURL URLWithString:_photo.placeholderUrl] placeholder:[UIImage imageNamed:@"default_image"]];

//        [_imageView yy_setImageWithURL:_photo.url placeholder:[UIImage imageNamed:@"default_image"]];
    }
    
//    {
//        _imageView.image = _photo.placeholder;
////        [_imageView sd_setImageWithURL:[NSURL URLWithString:_photo.placeholderUrl]];
//
//        [_imageView sd_setImageWithURL:[NSURL URLWithString:_photo.placeholderUrl] placeholderImage:[UIImage imageNamed:@"default_image"]];
...
- (void)adjustFrame
{
    ...
    NSLog(@"imageFrame0_%@",NSStringFromCGRect(imageFrame));
...
- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView {
    if (_zoomByDoubleTap) {
        ...
        if (ABS(_imageView.frame.origin.y - insetY) > 0.5) {
            ...
            NSLog(@"imageFrame1_%@",NSStringFromCGRect(imageViewFrame));
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale{
    ...
    if (ABS(_imageView.frame.origin.y - insetY) > 0.5) {
        [UIView animateWithDuration:0.2 animations:^{
            CGRect imageViewFrame = _imageView.frame;
            imageViewFrame = CGRectMake(imageViewFrame.origin.x, insetY, imageViewFrame.size.width, imageViewFrame.size.height);
            NSLog(@"imageFrame2_%@",NSStringFromCGRect(imageViewFrame));
            _imageView.frame = imageViewFrame;
        }];
    }
}
...
//    xcenter = scrollView.contentSize.width > scrollView.frame.size.width ? scrollView.contentSize.width/2 : c;
    xcenter = scrollView.contentSize.width/2;

    ycenter = scrollView.contentSize.height > scrollView.frame.size.height ? scrollView.contentSize.height/2 : ycenter;
    
    [_imageView setCenter:CGPointMake(xcenter, ycenter)];
    NSLog(@"imageView.center_%@",NSStringFromCGPoint(_imageView.center));
}
...
//长按
- (void)handleLongTap:(UITapGestureRecognizer *)tap {
