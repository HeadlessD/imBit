    ...
    if (pushToCamera) {
        ...
        if (model.type == MSTAssetModelMediaTypeVideo) {
            ...
            if ([pickerCtrler hasSelected]) {
                [self presentViewController:[self addAlertControllerWithTitle:NSLocalizedStringFromTable(@"str_both_video_photo", @"MSTImagePicker", @"不能同时选择视频和照片") actionTitle:NSLocalizedStringFromTable(@"str_i_see", @"MSTImagePicker", LLSTR(@"101023"))] animated:YES completion:nil];
            } else {
            ...
            if (self.isHideFullButtonAndImgPhotoGrid) {
                ppc.isHideFullButtonAndImgPhotoPreview = YES;
            }else{
                ppc.isHideFullButtonAndImgPhotoPreview = NO;
            }
